 ...
 * 但服务一重启这个 Map 就归零了 —— 于是出现下面这条无上限的循环：
 *
 ...
 *
 * 现在有两层限制：
 *   1. **进程内**：同一服务进程里同一会话最多续 1 次（`MAX_ATTEMPTS_PER_SESSION`）
 ...
 *
 * 记两个计数，用途不同（2026-09-23 修正后）：
 *   - `attempts`：**总**次数，只作观测用 —— 不再用它做上限。
 *   - `consecutive`：**连续未成功**次数，驱动自适应退避。
 *
 * 为什么把上限从"总次数"改成"连续失败次数"（实测教训）：
 * 原来总次数上限 3，于是出现了这条链 —— 续跑成功一次（任务继续）→ 又被打断 →
 * 总次数已 2 → 再失败一次就撞上限 → **网络恢复后也不再有第二次机会**。
 * 真正的失控风险是"反复失败"，不是"成功过很多次"。
 */
...
export function detectProgressSinceLastAttempt(data, sessionId, currentTurnSeq) {
  ...
  if (prevSeq === undefined) {
    // 向后兼容（2026-09-23 补）：旧版账本没有 `lastTurnSeq` 字段。
    // 上一版这里返回 success:false —— 于是**判据无声失效**：明明任务往前走过了，
    // 却仍按"连续失败"吃退避，最后把该续跑的挡在冷却里（实测踩到）。
    // 没有可比对的 seq 时，采取**对续跑有利**的解读：当作有进展处理（清零重来），
    // 因为"总尝试次数"这个硬上限与逐会话隔离仍然在防失控。
