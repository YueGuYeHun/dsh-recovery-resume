 ...
 *   「只扫 live agents」），而它跑在宿主启动后 3 秒 —— 那一刻**会话还没变成活
 ...
 * 跨重启账本的位置。
 *
 * 为什么需要它（我第一版的真实漏洞）：进程内的计数在宿主重启后归零，于是
...
export function apply(ctx) {
  ...
  function consider(agent, scoped) {
    ...
    const { data: ledger, error: ledgerError } = readLedger(LEDGER_PATH)
    if (ledgerError !== undefined) {
      console.error(`${PLUGIN}: 账本读取失败（按空账本处理）: ${ledgerError}`)
    }
    ...
    const info = inspectTail(events, Date.now())
    if (!info.resume) {
    ...
    try {
      message = createUserMessage({
        ...
        source: { kind: PLUGIN, plugin: PLUGIN, turnSeq: info.turnSeq },
      })
    } catch (error) {
    ...
    try {
      ...
      try {
        const now = Date.now()
        const updated = recordAttempt(pruneLedger(ledger, now), session.id, now)
        writeLedger(LEDGER_PATH, updated)
        console.log(
          `${PLUGIN}: 账本已更新 ${session.id} → 累计续跑 ${updated[session.id].attempts} 次` +
            `（跨重启上限 ${MAX_ATTEMPTS_ACROSS_RESTARTS}，冷却 ${COOLDOWN_MS / 1000}s）`,
        )
    ...
    // state, initially disarmed），每次宿主启动都会被 disarm —— 所以哪怕目标还在
