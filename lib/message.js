/**
 * 最小的 user 消息构造器 —— 零宿主依赖。
 *
 * 为什么不 import `@deepseek-ai/dsh-llm` 的 `createUserMessage`：
 *
 * 宿主包（`@deepseek-ai/dsh-llm`）不在本插件的 `dependencies` 里，装到别人机器上
 ...
 *    **冻结不是语义要求**：宿主自己的投影代码 `structuredClone` 输入，不要求预先冻结。
 * 5. **`source` 是纯数据**。宿主只用它做 `source.kind === 'plugin' &&
 ...
 *    —— 与宿主 `dsh-command-goal` 用 `createUserMessage` 时的写法一致。
 *
 ...
 *   这样宿主/客户端能认出这条消息是哪个插件投的（本插件据此判断"已处理"，
 *   不会把主人的消息误判成插件的）。
